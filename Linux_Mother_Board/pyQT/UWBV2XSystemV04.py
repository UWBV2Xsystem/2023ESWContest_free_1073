...
class UWB_V2X_System(QWidget):
    def __init__(self):
        ...
        # Pedestrian Img Location
        self.PedestrianLeft = [500, 150, 203, 212]
        self.PedestrianRight = [1200, 150, 203, 212]
        ...
        # Pedestrian
        self.Uart_Pedestrian_x = 0
        self.Uart_Pedestrian_y = 0

        self.Pedestrian_moving_x = []
        self.Pedestrian_moving_y = []

        # data

        self.Uart_Car = None
        self.Uart_Pedestrian = None
        ...
        self.Pedestrian_Img = QLabel(self)
    ...
    def RunTimer(self):
        ...
        self.timer.timeout.connect(self.DetectPedestrian)
                    ...
                    if self.In_Uart[-1] == '\n':
                        ...
                        try:
                            if self.Uart_data['type'] == 'Tli':  # if Traffic Light
                                ...
                                self.Uart_data = {}
                            elif self.Uart_data['type'] == 'Ped':  # if Pedestrian
                                self.Uart_Pedestrian = self.Uart_data
                                self.Uart_data = {}
                        except:
                            pass
                except:
    ...
    def GetDataFromUart(self):  # QTimer
        try:
            ...
            self.Uart_Pedestrian_x = round(float(self.Uart_Pedestrian['X']), 2)
            self.Uart_Pedestrian_y = round(float(self.Uart_Pedestrian['Y']), 2)
            ...
            self.Pedestrian_moving_x, self.Uart_Pedestrian_x = \
                MovingAverage(self.Pedestrian_moving_x, self.Uart_Pedestrian_x)
            self.Pedestrian_moving_y, self.Uart_Pedestrian_y = \
                MovingAverage(self.Pedestrian_moving_y, self.Uart_Pedestrian_y)
    ...
    def DetectPedestrian(self):  # QTimer
        try:
            # out road
            if abs(self.Uart_Pedestrian_x) > X_ped_matrix_P:
                self.Pedestrian_Img.hide()

            else:  # in road
                if 0 < self.Uart_Pedestrian_x:  # Right
                    pixmap = QPixmap(img_dir + '/Pedestrian_Right.png').scaled(203, 212)
                    self.Pedestrian_Img.setGeometry(*self.PedestrianRight)
                elif 0 > self.Uart_Pedestrian_x:  # Left
                    pixmap = QPixmap(img_dir + '/Pedestrian_Left.png').scaled(203, 212)
                    self.Pedestrian_Img.setGeometry(*self.PedestrianLeft)

                self.Pedestrian_Img.setPixmap(pixmap)
                self.Pedestrian_Img.show()
